...
2. **Do not rehydrate for outbound.** If you need to show the user a real value on their screen (not in an outbound payload), call `mcp__piighost__rehydrate_text` on the preview string only. The outbound payload keeps the placeholder.
3. **If the user types a real name in chat**, call `mcp__piighost__anonymize_text` on any text you incorporate into an outbound draft, including the user's words, before sending.
